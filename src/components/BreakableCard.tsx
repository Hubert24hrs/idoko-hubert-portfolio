...
import styles from './BreakableCard.module.css';
...
const useBreakableCard = (onBreak?: () => void) => {
    ...
    const [fallState, setFallState] = useState<FallState>({
        active: false, velocityX: 0, velocityY: 0, rotation: 0,
    });
    ...
    return {
        isDragging, position, crackLevel, isBroken, fallState,
        debrisChunks, handleDragStart, respawnProgress, isFlashing,
    };
};
...
const CrackLines = ({ level }: { level: number }) => {
    ...
    return (
        <svg className={styles.crackSvg} viewBox="0 0 100 100" preserveAspectRatio="none">
            {level >= 0.1 && (
                <path d="M0 20 L15 25 L8 40 L20 50" fill="none" stroke="black" strokeWidth="2"
                    vectorEffect="non-scaling-stroke" style={{ opacity: Math.min(1, level * 2) }} />
            )}
            {level >= 0.5 && (
                <path d="M50 100 L55 80 L45 70 L60 55" fill="none" stroke="black" strokeWidth="2"
                    vectorEffect="non-scaling-stroke" style={{ opacity: Math.min(1, (level - 0.4) * 2) }} />
            )}
            {level >= 0.9 && (
                <path d="M20 0 L25 20 L40 25 L35 45 L50 50" fill="none" stroke="black" strokeWidth="3"
...
export const BreakableCard = ({ title, description, className, onBreak }: SectionCardProps) => {
    ...
    return (
        <div className={cn(styles.wrapper, className)}>
            {/* Shadow remnant background */}
            <div className={styles.shadowRemnant}>
                <span>Gone</span>
                {isBroken && (
                    <div className={styles.respawnBar}>
                        <div className={styles.respawnFill} style={{ width: `${respawnProgress}%` }} />
                    </div>
                )}
            </div>
            ...
            <div
                ...
                className={cn(
                    styles.card,
                    !isDragging && !isBroken && styles.cardIdle,
                    isBroken && styles.cardBroken,
                    isFlashing && styles.cardFlashing,
                )}
                style={{
                    transform: isBroken
                        ? fallTransform
            ...
            >
                ...
                <div className={styles.cardFooter}>
                    ...
                    <span className={crackLevel > 0.7 ? styles.cardFooterDanger : undefined}>
                        {crackLevel > 0 ? `${Math.round(crackLevel * 100)}% DMG` : 'INTACT'}
                    </span>
                </div>
                <CrackLines level={crackLevel} />
                {crackLevel > 0.2 && (
                    <div className={styles.damageOverlay} style={{ opacity: crackLevel * 0.15 }} />
                )}
            </div>
